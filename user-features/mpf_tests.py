...
def mpf2(data):
    ...
    for i in datawt.columns:
        gg.append(f"acc_{i}_mpf2")
        fvec, dff = fft_sig(data[i])
        value = sum(p * f for p,f in zip(dff, fvec)) / sum(dff)
        mean_power_frequency.append(value)
...
def feats_df(data):
    ...
    try:
        ...
        #        )
        mpf_ = (*mpf(data), *mpf1(data), *mpf2(data))
...
da = pd.read_csv('Accelerometer_1.csv')
plt.plot(da['seconds_elapsed'], da['x'])
plt.show()
n = len(da)
