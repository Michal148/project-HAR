...
def quatmag(mx, my, mz, qa):
    ...
    # Equation(28)
    gamma_ = rotated_mag_field[0]**2 + rotated_mag_field[1]**2

    # Equation(35)
    if rotated_mag_field[0] >= 0:
        matrix = np.array([np.sqrt((gamma_+rotated_mag_field[0]*np.sqrt(gamma_)) / np.sqrt(2*gamma_)), 0, 0,
                           rotated_mag_field[1] / np.sqrt(2*np.sqrt(gamma_+rotated_mag_field[0]*np.sqrt(gamma_)))])
    else:
        matrix = np.array([rotated_mag_field[1] / np.sqrt(2*np.sqrt(gamma_-rotated_mag_field[0]*np.sqrt(gamma_))),
                           0, 0, np.sqrt((gamma_-rotated_mag_field[0]*np.sqrt(gamma_)) / np.sqrt(2*gamma_))])
...
def orientation(accel, magne, gyro):
    ...
    for id_ in range(len(acc) - 1):
        ...
        if cos_omega > threshold:
            ...
            norm_delta_q = np.array([delta_q_line[0] / norm_nums, delta_q_line[1] / norm_nums,
                                    delta_q_line[2] / norm_nums, delta_q_line[3] / norm_nums])
        else:
            omega = np.arccos(cos_omega)
            norm_delta_q = np.sin((1-alpha)*omega)/np.sin(omega)*quat_id + np.sin(alpha*omega)/np.sin(omega)*delta_q_acc

        # quaternion estimated from gyroscope data is multiplied by the filtered delta quaternion,
        # based on Equation (53) from [1]
        ...
        orientationList.append(q_t)

    orientationDf = pd.DataFrame(orientationList, columns = ['q0', 'q1', 'q2', 'q3'])
    return orientationDf
